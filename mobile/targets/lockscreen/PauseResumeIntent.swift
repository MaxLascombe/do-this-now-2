...
// The lock-screen button. Conforms to LiveActivityIntent so the system runs
// perform() in the APP process — usually already alive in the background —
// instead of cold-spawning the widget-extension process on every tap (that
// spawn was a visible ~1s of dead time before the optimistic flip). The
// type must be compiled into BOTH the app and the extension for that to
// work; an identical copy lives in modules/lock-screen-bridge/ios — keep
// the two files byte-identical (same pattern as LockScreenTimerAttributes).
//
// POSTs to the server with the device token the app parked in the shared
// App Group (ADR-0004), then applies the returned state to the local
  ...
  func perform() async throws -> some IntentResult {
    ...
    do {
      ...
      if response.state == nil {
        // Literal kind: PROGRESS_WIDGET_KIND lives in the extension target
        // only, and this file compiles into both.
