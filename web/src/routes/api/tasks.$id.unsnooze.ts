...
import { TaskNotFoundError, unsnoozeTask } from '../../server/lib/actions'
...
export const Route = createFileRoute('/api/tasks/$id/unsnooze')({
  server: {
    handlers: {
      POST: withAuth<Params>(async ({ userId, params }) => {
        if (!idSchema.safeParse(params.id).success) return notFound()
        try {
          return json(await unsnoozeTask(userId, params.id))
        } catch (e) {
